...
// 跳转用户信息页面
(function initialize() {
     App.goToUserMessage = function (app_data) {
         window.webkit.messageHandlers.goToUserMessage.postMessage(app_data);
     };
 })();

// 跳转用户店铺
(function initialize() {
     App.goToUserStore = function (app_data) {
         window.webkit.messageHandlers.goToUserStore.postMessage(app_data);
     };
 })();

// 跳转新增商品
(function initialize() {
     App.add_goods = function (app_data) {
         window.webkit.messageHandlers.add_goods.postMessage(app_data);
     };
 })();

// IM客服(类似于私聊)
(function initialize() {
     App.go_chat = function (app_data) {
         window.webkit.messageHandlers.go_chat.postMessage(app_data);
     };
 })();

// 回到app根视图
(function initialize() {
     App.showNativeHomePage = function (app_data) {
         window.webkit.messageHandlers.showNativeHomePage.postMessage(app_data);
     };
 })();

// 编辑按钮
(function initialize() {
     App.edit_goods = function (app_data) {
         window.webkit.messageHandlers.edit_goods.postMessage(app_data);
     };
 })();
